...
const captchaDialog = document.querySelector('#captcha-dialog');
...
function openCaptcha(onSuccess) { const first = Math.floor(Math.random() * 7) + 2; const second = Math.floor(Math.random() * 7) + 2; captchaAnswer = first + second; captchaQuestion.textContent = `${first} + ${second} = ?`; captchaInput.value = ''; captchaError.textContent = ''; captchaDialog.hidden = false; captchaDialog.dataset.next = String(onSuccess); setTimeout(() => captchaInput.focus(), 0); }
function verifyCaptcha() { if (Number(captchaInput.value) !== captchaAnswer) { captchaError.textContent = 'Please enter the correct answer.'; captchaInput.focus(); return; } captchaDialog.hidden = true; showStep(Number(captchaDialog.dataset.next)); }
document.querySelector('#captcha-verify')?.addEventListener('click', verifyCaptcha);
captchaInput?.addEventListener('keydown', event => { if (event.key === 'Enter') { event.preventDefault(); verifyCaptcha(); } });
...
document.querySelectorAll('[data-next]').forEach(button => button.addEventListener('click', () => { const current = document.querySelector(`.form-step[data-step="${currentStep}"]`); if (!current.querySelectorAll('input:invalid, select:invalid').length) { const next = Number(button.dataset.next); if (next === 3) openCaptcha(next); else showStep(next); } else current.querySelector('input:invalid, select:invalid')?.reportValidity(); }));
